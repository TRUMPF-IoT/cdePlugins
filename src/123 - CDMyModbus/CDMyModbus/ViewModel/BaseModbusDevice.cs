...
{
    public class TheBaseModbusDevice : TheThingBase
    {
        public TheBaseModbusDevice(TheThing tBaseThing, ICDEPlugin pPluginBase, DeviceDescription pModDeviceDescription)
        {
            if (tBaseThing != null)
        ...
        {
            sinkPChanged(new cdeP(pName, pValue));
        ...
        protected ModbusMaster MyModMaster;
        ...
        public override bool Init()
        {
            ...
            MyBaseEngine.RegisterEvent(eEngineEvents.ShutdownEvent, sinkshutdown);
            MyModFieldStore.InitializeStore(true, false);
            return false;
        }

        void sinkshutdown(ICDEThing psender, object nouse)
        {
            CloseModBus();
        }
        ...
        public virtual string OpenModBus()
        {
            return "not implemented - must be overridden by subclass";
        }

        public virtual void CloseModBus()
        {
        }

        public virtual void sinkStoreReady(StoreEventArgs e)
        {
            if (MyDevice != null)
            {
        ...
        protected virtual bool Connect(TheProcessMessage pMsg)
        {
            bool bSuccess = false;
            try
            {
        ...
        protected virtual bool Disconnect(TheProcessMessage pMsg)
        {
            CloseModBus();
            MyBaseThing.StatusLevel = 0;
        ...
        protected void sinkPChanged(cdeP prop)
        {
            ...
            if (field == null || !field.AllowWrite) return;
        ...
        public async void ReaderThread()
        {
                    ...
                    if (!MyBaseEngine.GetEngineState().IsSimulated)
                    {
                        ...
                        else
                        {
                            try
                            {
                                ...
                                MyBaseThing.SetStatus(1, $"{timestamp} - {dict?.Count} tags read from Modbus Device");
            ...
            {
                SetMessage($"{DateTime.Now} - Error during read or processing of modbus properties: {e.Message}",2, DateTimeOffset.Now, 10483);
            }
            finally
            {
                lock (readerLoopLock)
        ...
        public virtual ushort[] ReadOneField(int tReadWay, byte tSlaveAddress, ushort address, FieldMapping field, Dictionary<string, object> dict)
        {
            ushort[] data = null;
            switch (tReadWay)
            {
        ...
        public Dictionary<string, object> ReadAll()
        {
            ...
            byte tSlaveAddress = (byte)SlaveAddress;
            int tReadWay = ConnectionType;
            foreach (var field in MyModFieldStore.TheValues)
            {
                try
                {
                    if (field.PollCycle > 1 && field.CurCycle < field.PollCycle)
                    {
                        field.CurCycle++;
                        continue;
                    }
                    ...
                    data = ReadOneField(tReadWay, tSlaveAddress, (ushort)address, field, dict);
        ...
        protected virtual void DoCreateUX()
        {

        }
        protected TheFormInfo MyFldMapperTable = null;
        protected TheFormInfo MyModConnectForm = null;
        protected TheDashPanelInfo MyDashIcon = null;
        public override bool CreateUX()
        {
            if (mIsUXInitCalled) return false;
            mIsUXInitCalled = true;

            var tFlds = TheNMIEngine.AddStandardForm(MyBaseThing, MyBaseThing.FriendlyName, 18, null, null, 0, ".Modbus Device");
            MyModConnectForm = tFlds["Form"] as TheFormInfo;
            MyDashIcon = tFlds["DashIcon"] as TheDashPanelInfo;
            ...
            TheNMIEngine.AddSmartControl(MyBaseThing, MyModConnectForm, eFieldType.Number, 207, 2, 0, "Slave Address", nameof(SlaveAddress), new nmiCtrlSingleEnded() { TileWidth = 3, ParentFld = 200 });
            ...
            TheNMIEngine.AddSmartControl(MyBaseThing, MyFldMapperTable, eFieldType.Number, 85, 2, 0, "Poll every", "PollCycle", new nmiCtrlNumber() { TileWidth = 1, FldWidth = 1 });
        ...
        public override void HandleMessage(ICDEThing sender, object pIncoming)
        {
            ...
            switch (cmd[0])
            {
                ...
                case nameof(TheThing.MsgSubscribeSensors):
                    ...
                    foreach (TheThing.TheSensorSubscription sub in subscribeRequest.SubscriptionRequests)
                    {
                        ...
                        if (sub.ExtensionData != null)
                        {
                            ...
                            if (sub.ExtensionData.TryGetValue("PollCycle", out var pullcycle))
                                fld.PollCycle = TheCommonUtils.CInt(pullcycle);
                ...
                case "WRITE_TAG":
                    {
                        sinkPChanged(new cdeP(cmd[1], pMsg.Message.PLS));
                    }
                    break;
                default:
                    break;
