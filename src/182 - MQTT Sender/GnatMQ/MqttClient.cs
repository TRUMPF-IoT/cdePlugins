...
//#else
                ...
                {
                    // this.keepAliveEventEnd.WaitOne(); // this used to hang sometimes!
                    int waitCounter = 0;
                    const int maxWait = 300;
                    while (this.keepAliveEventEnd?.WaitOne(100) == false && waitCounter < maxWait)
                    {
                        this.keepAliveEvent?.Set();
                        waitCounter++;
                    } 
                    ...
                    {
#if TRACE
                        MqttUtility.Trace.WriteLine(TraceLevel.Error, "KeepAlive thread not confirmed to have stopped");
#endif
                    }
                }
            }
...
//#endif
        ...
        private void ReceiveThread()
        {
            try
            {
                ...
                while (this.isRunning)
                {
                    try
                    {
                        ...
                        {
                            //#if BROKER
                            //if (!_RunAsBroker) // Need to do this also for broker so keep alives are handled properly
                            {
            ...
            {
                // Make sure we don't take down the process
#if TRACE
                try
                {
                    MqttUtility.Trace.WriteLine(TraceLevel.Error, "Fatal exception occurred in receive loop: {0}", e.ToString());
                }
                catch { }
#endif
            }
        ...
        object keepAliveStartLock = new object();
        /// <summary>
        ...
        private void KeepAliveThread()
        {
            try
            {
                lock (keepAliveStartLock)
                {
                    ...
                    {
#if TRACE
                        MqttUtility.Trace.WriteLine(TraceLevel.Error, "Multiple keep alive threads started!");
#endif
                        return;
                    }
                }
        ...
        private void ProcessInflightThread()
        {
            ...
            try
            {
                while (this.isRunning)
                {
                    ...
                    while (!this.inflightWaitHandle.WaitOne(timeout == Timeout.Infinite || timeout > 30000 ? 30000 : timeout) && this.isRunning && (timeout == Timeout.Infinite || sw.ElapsedMilliseconds < timeout) )
                    {
                    }
                    sw.Stop();
#endif
                                ...
                                switch (msgContext.State)
                                {
                                    ...
                                    case MqttMsgState.WaitForUnsuback:
                                        ...
                                        if (msgContext.Flow == MqttMsgFlow.ToPublish)
                                        {
                                            ...
                                            lock (this.internalQueue)
                                            {
                                                // Bug #1289: This lead to the bahaviour that not all messages in internalqueue are processed.
                                                //if (this.internalQueue.Count > 0)
                                                //    msgReceived = (MqttMsgBase)this.internalQueue.Peek();
                                                if (internalQueue.Count > 0)
                                                {
                                                    msgReceived = SeekAndPeekQueue(internalQueue,
                                                        msgInflight.MessageId) as MqttMsgBase;
                                                }
                                            }
            ...
            catch (Exception e)
            {
                ...
                try
                {
                    MqttUtility.Trace.WriteLine(TraceLevel.Error, "Fatal exception occurred in ProcessInflightThread: {0}", e.ToString());
                }
                catch { }
#endif
            }
        ...
        private object SeekAndPeekQueue(Queue queue, ushort messageId)
        {
            var msgArray = queue.ToArray();
            return msgArray.FirstOrDefault((o) =>
            {
                var msg = (MqttMsgBase) o;
