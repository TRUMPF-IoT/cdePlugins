...
namespace uPLibrary.Networking.M2Mqtt
{
    ...
    public class MqttBrokerWithReceiverCallback : MqttBroker
    {
        public MqttBrokerWithReceiverCallback() : base() { }
        public MqttBrokerWithReceiverCallback(IMqttCommunicationLayer commLayer, MqttSettings settings) : base(commLayer, settings) { }

        public Action<string,byte[]> Receiver { get; set; }
        ...
        protected override void Client_MqttMsgPublishReceived(object sender, MqttMsgPublishEventArgs e)
        {
            try
            {
                ...
                Receiver?.Invoke(topic, payload);
            }
            catch (Exception)
            {
                //TheBaseAssets.MySYSLOG.WriteToLog(700, TSM.L(eDEBUG_LEVELS.OFF) ? null : new TSM(MyBaseThing.EngineName, "MQTT Receiver: internal error receiving Message", eMsgLevel.l1_Error, ex.ToString()));
            }
    ...
    public class MqttBroker
    {
        ...
        {
            this.commLayer.Start(synchronous);
